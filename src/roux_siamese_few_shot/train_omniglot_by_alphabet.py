...
from roux_siamese_few_shot.datasets.omniglot_by_alphabet import OmniglotByAlphabet
from roux_siamese_few_shot.datasets.pairs.omniglot import create_omniglot_pairs_alphabet
...
def main():
    ...
    omniglot_dataset = OmniglotByAlphabet(root='data',
                                          background=True,
                                          download=True,
                                          transform=omniglot_transform)

    pairs = create_omniglot_pairs_alphabet(omniglot_dataset, 2000)

    kwargs = {'num_workers': 1, 'pin_memory': True} if args.cuda else {}

    train_loader = DataLoader(Dataset(*pairs),
    ...
    omniglot_test_dataset = OmniglotByAlphabet(root='data',
                                               background=False,
                                               download=True,
                                               transform=omniglot_transform)
